...
public class OperatorsDataTypes {
  static void Main(string[] args){
    // Print a header line
    Console.WriteLine("\nMichael Corleone - Week 1 GP Operators & Data Types\n");
    ...
    //Greater Than
    if(intVal2 > intVal1) {
      Console.WriteLine("{0:D} is greater than {1:D}", intVal2, intVal1);
    }
    ...
    val = Console.ReadLine();
    double fitVal1 = Convert.ToDouble(val);
    Console.Write("Enter a second floating point value: ");
    val = Console.ReadLine();
    double fitVal2 = Convert.ToDouble(val);
    double total = fitVal1 + fitVal2;
    Console.WriteLine("The sum of {0:F3} and {1:F3} is {2:F3}.", fitVal1, fitVal2, total);
  }
}